...
## Iteration 4 (Feb–Jul 2026) — VPS Production
- ✅ Deployment automation: `deploy.sh` (bash) + `DEPLOY.md`. Local MongoDB, PM2, Nginx reverse proxy, SSL via certbot. Auto-seeds admin + settings.
- ✅ Dynamic admin URL via `REACT_APP_ADMIN_BASE` (defaults to `/admin` in dev, e.g. `/secret-panel` in prod)
- ✅ Same-origin API (`REACT_APP_BACKEND_URL=""` in prod) so app works under HTTP and HTTPS via Nginx proxy
- ✅ Google Analytics via `REACT_APP_GA_ID` env var + script tag in `index.html`
- ✅ **Native Live Traffic Analytics**: `POST /api/track` beacon (fires on route change + 30s heartbeat, sendBeacon fallback), `GET /api/admin/analytics` returns active_now, on_home/product/cart/checkout, visitors_today/7d/30d, page_views_today, top_pages_7d. TTL indexes purge old `page_views` (30 days) and `live_sessions` (24h). Dashboard shows real-time widget with 5s polling and now displays exact HTTP/detail error inline if the API call fails.
- ✅ **All external image URLs migrated to same-origin `/img/*` bundled in `frontend/public/img/`** — hero + 7 gallery images downloaded locally, optimized (3 MB pexels → 230 KB), and DB migration `migrate_external_images()` rewrites legacy `customer-assets.emergentagent.com` / `unsplash` / `pexels` URLs on backend startup. Fixes visitors on ISPs/adblockers that block those CDNs.
- ✅ **Admin Lifestyle CRUD**: new `LifestyleItem` model + `db.lifestyle` collection + `/api/lifestyle` (public) + `/api/admin/lifestyle` (POST/GET/PUT/DELETE). Frontend `Lifestyle.jsx` fetches from API; new `pages/admin/Lifestyle.jsx` mirrors the Gallery admin UX. Seed pre-populates 8 default scenes.
- ✅ **Multi-image product support**: `ProductInfo.images: List[str] = []` field. New `MultiImageUpload.jsx` component (multi-select upload, URL paste, drag-to-reorder via ←/→ buttons, "Set as Main" star, per-tile remove, up to 12 images, first image = main). Admin → Product now uses it. Public **Product page** shows main image + tap-to-switch thumb strip. **Homepage Hero** auto-slides every 3.8s with opacity crossfade + dot indicators (only when 2+ images). Pauses when tab hidden.
- ✅ **SMTP diagnostics + SSL/TLS/Auto selector**: `send_email_with_error()` returns specific messages (auth failure with Gmail App Password link, network error, TLS/SSL mismatch, wrong host, missing from_email, etc.). `POST /api/admin/smtp/test` accepts inline `smtp` config so admin can test WITHOUT saving first. New `SMTPSettings.security: "auto"|"tls"|"ssl"|"none"` field — "auto" picks SSL for port 465, STARTTLS for 587/25, and retries the other on protocol errors. Admin UI now shows an Encryption dropdown (Auto default) with port auto-suggest (465↔587) when switching modes.
