      ...
      <div className="max-w-[1180px] mx-auto px-4 sm:px-6 lg:px-10 grid lg:grid-cols-[1fr_420px] lg:gap-16 xl:gap-24">
        ...
        <main data-testid={TID.thankYou} className="py-8 lg:py-12">
          ...
          <div className="flex items-start gap-4 mb-9">
            <div className="w-12 h-12 rounded-full bg-amber-100 border border-amber-500/40 flex items-center justify-center shrink-0 pulse-ring">
            ...
            <div>
              ...
              <p className="text-sm text-[#6D7175] mt-2 max-w-xl">
          ...
          {/* Order updates card */}
          <div className="bg-white border border-[#E1E3E5] rounded-2xl p-5 mb-6">
          ...
          {order?.tracking_token && (
            <Link
              ...
              className="block bg-white border border-amber-500/40 hover:border-amber-500 rounded-2xl p-5 mb-6 transition-all group"
            >
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <h3 className="font-display text-lg text-[#202223]">Track your order</h3>
                  <p className="text-sm text-[#6D7175] mt-0.5">Open the live tracking page for {order.order_number}</p>
                </div>
                <ChevronRight size={18} className="text-[#8C9196] group-hover:text-amber-500 group-hover:translate-x-1 transition-all" />
              </div>
            </Link>
          )}
            ...
            <section className="bg-white border border-[#E1E3E5] rounded-2xl p-5 sm:p-6 mb-6">
              ...
              <div className="grid sm:grid-cols-2 gap-6">
                <InfoBlock title="Contact information" icon={Mail}>
                  <div>{order.email}</div>
                  <div className="text-[#8C9196] text-xs mt-1 flex items-center gap-1.5"><Phone size={11} /> {order.phone}</div>
                </InfoBlock>
                <InfoBlock title="Payment method" icon={CreditCard}>
                  <div>{pmLabel(order.payment_method)}</div>
                  <div className="text-[#8C9196] text-xs mt-1 capitalize">{order.payment_status === "paid" ? "Paid" : `Status: ${order.payment_status}`}</div>
                </InfoBlock>
                <InfoBlock title="Shipping address" icon={MapPin}>
                  <div>{order.full_name}</div>
                  <div className="text-[#6D7175]">{order.address}</div>
                  <div className="text-[#6D7175]">{order.city}, {order.state} {order.pincode}</div>
                  {order.landmark && <div className="text-[#8C9196] text-xs mt-1">Landmark: {order.landmark}</div>}
                </InfoBlock>
                <InfoBlock title="Shipping method" icon={Truck}>
                  <div>Standard Delivery</div>
                  <div className="text-[#8C9196] text-xs mt-1">Arrives in 4–7 business days</div>
                </InfoBlock>
              </div>
            </section>
          )}
          ...
          {/* Help */}
          <div className="bg-white border border-[#E1E3E5] rounded-2xl p-5 sm:p-6 mb-8">
            <h2 className="font-display text-lg mb-1">Need help?</h2>
            <p className="text-sm text-[#6D7175] mb-4">Reach out to our team — we usually reply within an hour.</p>
            <div className="flex flex-wrap gap-2">
              ...
              <Link to="/track-order" className="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-[#E1E3E5] hover:border-amber-500 hover:text-amber-500 text-sm">
                <Truck size={14} /> Track your order
              </Link>
            </div>
          </div>

          <div className="flex items-center justify-between flex-wrap gap-3 border-t border-[#E1E3E5] pt-6">
...
function InfoBlock({ title, icon: Icon, children }) {
  return (
    <div>
      <div className="text-xs uppercase tracking-widest text-[#8C9196] flex items-center gap-1.5 mb-2">
        <Icon size={12} /> {title}
      </div>
      <div className="text-sm text-[#202223] space-y-0.5">{children}</div>
    </div>
  );
}
    ...
    <div className="space-y-5">
      ...
      <div className="space-y-2 text-sm pt-3 border-t border-[#E1E3E5]">
        ...
        <div className="flex justify-between items-baseline pt-3 mt-1 border-t border-[#E1E3E5]">
          <span className="text-xs uppercase tracking-widest text-[#6D7175]">Total paid</span>
          <div>
