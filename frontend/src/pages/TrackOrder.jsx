...
function TimelineRow({ step, status, last }) {
  ...
  return (
    <li className="flex gap-4 relative">
      {!last && <span className={`absolute left-[14px] top-8 bottom-[-24px] w-px ${done ? "bg-amber-500/60" : "bg-[#E1E3E5]"}`} />}
      <div
        className={`relative z-10 w-7 h-7 rounded-full flex items-center justify-center shrink-0 border-2 ${
            ...
            : active ? "border-amber-500 bg-amber-50 text-amber-500"
            : "border-[#E1E3E5] bg-white text-[#8C9196]"
        }`}
      >
        <Icon size={13} />
      </div>
      <div className={`flex-1 pb-6 ${last ? "pb-0" : ""}`}>
        <div className="flex items-baseline justify-between gap-2 flex-wrap">
          <p className={`text-sm font-medium ${active ? "text-amber-500" : done ? "text-[#202223]" : "text-[#8C9196]"}`}>
            {step.label}
          </p>
          {at && <span className="text-[11px] text-[#8C9196]">{new Date(at).toLocaleString()}</span>}
        </div>
        <p className="text-xs text-[#8C9196] mt-0.5">{note || step.desc}</p>
      </div>
    </li>
  );
}
      ...
      <div className="min-h-screen bg-white text-[#202223]">
        ...
        <main className="max-w-md mx-auto px-4 pt-12 pb-20">
          <p className="text-xs text-[#8C9196] mb-4">
            ...
            <ChevronRight size={11} className="inline mx-1 -mt-0.5 text-[#C9CCCF]" />
            <span>Track order</span>
          </p>
          <h1 className="font-display text-3xl sm:text-4xl mb-2">Where's my order?</h1>
          <p className="text-sm text-[#6D7175] mb-8">Enter your order number and the phone number used at checkout.</p>
          <form onSubmit={submit} className="space-y-3">
            <input
              data-testid={TID.trackInput}
              className="w-full h-14 bg-white border border-[#8C9196] rounded-lg px-4 text-[15px] font-mono tracking-wider placeholder:text-[#8C9196] focus:border-amber-500 focus:outline-none"
            ...
            <input
              ...
              className="w-full h-14 bg-white border border-[#8C9196] rounded-lg px-4 text-[15px] placeholder:text-[#8C9196] focus:border-amber-500 focus:outline-none"
          ...
          <p className="text-xs text-[#8C9196] mt-6 text-center">
      ...
      <div className="max-w-[1180px] mx-auto px-4 sm:px-6 lg:px-10 grid lg:grid-cols-[1fr_420px] lg:gap-16 xl:gap-24">
        ...
        <main className="py-8 lg:py-12">
          ...
          <div className="mb-7">
            ...
            <p className="text-sm text-[#6D7175] mt-2 max-w-xl">{head.sub}</p>
          </div>

          {/* Progress bar pseudo-map */}
          {order.status !== "cancelled" && (
            <div className="bg-white border border-[#E1E3E5] rounded-2xl p-5 mb-6 overflow-hidden relative">
              <div className="flex items-center justify-between mb-3 text-[11px] text-[#8C9196]">
                <span>Placed</span><span>Delivered</span>
              </div>
              <div className="h-2 bg-[#E1E3E5] rounded-full overflow-hidden mb-4">
                <div
                ...
                {STEPS.map((step, i) => {
                  ...
                  return (
                    <div key={step.key} className="flex flex-col items-center text-center">
                      <div className={`w-7 h-7 rounded-full flex items-center justify-center border ${done ? "bg-amber-500 border-amber-500 text-black" : "border-[#E1E3E5] text-[#8C9196]"}`}>
                        <Icon size={12} />
                      </div>
                      <div className={`text-[9px] mt-1.5 leading-tight ${done ? "text-[#202223]" : "text-[#8C9196]"} hidden sm:block`}>
                        {step.label}
                      </div>
                    </div>
                  );
          ...
          {/* Detailed timeline */}
          <section className="bg-white border border-[#E1E3E5] rounded-2xl p-5 sm:p-6 mb-6">
          ...
          <section className="bg-white border border-[#E1E3E5] rounded-2xl p-5 sm:p-6 mb-6">
            ...
            <div className="grid sm:grid-cols-2 gap-6 text-sm">
              <div>
                <div className="text-xs uppercase tracking-widest text-[#8C9196] mb-1.5 flex items-center gap-1.5"><Mail size={11} /> Contact</div>
                <div>{order.email}</div>
                <div className="text-[#8C9196] text-xs mt-0.5 flex items-center gap-1.5"><Phone size={11} /> {order.phone}</div>
              </div>
              <div>
                <div className="text-xs uppercase tracking-widest text-[#8C9196] mb-1.5 flex items-center gap-1.5"><MapPin size={11} /> Shipping to</div>
                <div>{order.full_name}</div>
                <div className="text-[#6D7175] text-xs">{order.address}</div>
                <div className="text-[#6D7175] text-xs">{order.city}, {order.state} {order.pincode}</div>
              </div>
              <div>
                <div className="text-xs uppercase tracking-widest text-[#8C9196] mb-1.5">Payment</div>
                <div>{pmLabel(order.payment_method)}</div>
                <div className="text-[#8C9196] text-xs mt-0.5 capitalize">{order.payment_status}</div>
              </div>
              <div>
                <div className="text-xs uppercase tracking-widest text-[#8C9196] mb-1.5">Placed on</div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            {s?.whatsapp_number && (
              <a href={`https://wa.me/${s.whatsapp_number.replace(/\D/g, "")}`} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-[#E1E3E5] hover:border-amber-500 hover:text-amber-500 text-sm">
                <MessageCircle size={14} /> WhatsApp support
              </a>
            )}
            <Link to="/" className="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-[#E1E3E5] hover:border-amber-500 hover:text-amber-500 text-sm">
        ...
        <div className="lg:hidden border-t border-[#E1E3E5] px-4 py-8">
          <Summary order={order} s={s} />
        </div>
      </div>
    </div>
...
function Summary({ order, s }) {
  return (
    <div className="space-y-5">
      ...
      <div className="space-y-2 text-sm pt-3 border-t border-[#E1E3E5]">
        <div className="flex justify-between"><span className="text-[#6D7175]">Subtotal</span><span>${order.subtotal.toFixed(2)}</span></div>
        {order.discount > 0 && (
          <div className="flex justify-between text-amber-500"><span>Discount</span><span>−${order.discount.toFixed(2)}</span></div>
        )}
        <div className="flex justify-between"><span className="text-[#6D7175]">Shipping</span><span>{order.shipping === 0 ? <span className="text-amber-500">Free</span> : `$${order.shipping.toFixed(2)}`}</span></div>
        <div className="flex justify-between items-baseline pt-3 mt-1 border-t border-[#E1E3E5]">
